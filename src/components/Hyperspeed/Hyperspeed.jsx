...
import { Scene, Vector3, Vector2,Vector4, PerspectiveCamera, WebGLRenderer,Fog, Clock ,LoadingManager,LineCurve3,TubeGeometry,InstancedBufferGeometry,Color,InstancedBufferAttribute, ShaderMaterial, Mesh,ShaderChunk ,PlaneGeometry, DoubleSide,  } from "three";

import { BloomEffect, EffectComposer, EffectPass, RenderPass, SMAAEffect, SMAAPreset, } from "postprocessing";

import "./Hyperspeed.css";

const HyperspeedComponent = ({
  effectOptions = {
    onSpeedUp: () => {},
    onSlowDown: () => {},
  ...
  useEffect(() => {
    ...
    const distortions = {
      mountainDistortion: {
        ...
        getJS: (progress, time) => {
          ...
          let lookAtAmp = new Vector3(2, 2, 2);
          let lookAtOffset = new Vector3(0, 0, -5);
      ...
      xyDistortion: {
        ...
        getJS: (progress, time) => {
          ...
          let lookAtAmp = new Vector3(2, 0.4, 1);
          let lookAtOffset = new Vector3(0, 0, -3);
          return distortion.multiply(lookAtAmp).add(lookAtOffset);
        },
      },
      LongRaceDistortion: {
        ...
        getJS: (progress, time) => {
          ...
          let lookAtAmp = new Vector3(1, 1, 0);
          let lookAtOffset = new Vector3(0, 0, -5);
          return distortion.multiply(lookAtAmp).add(lookAtOffset);
        },
      },
      turbulentDistortion: {
        ...
        getJS: (progress, time) => {
          ...
          let lookAtAmp = new Vector3(-2, -5, 0);
    ...
    class App {
      constructor(container, options = {}) {
        ...
        let fog = new Fog(options.colors.background, options.length * 0.2, options.length * 500);
        this.scene.fog = fog;
        this.fogUniforms = {
    ...
    class CarLights {
      ...
      init() {
        ...
        let geometry = new TubeGeometry(curve, 40, 1, 8, false);
    ...
    class Road {
      ...
      createPlane(side, width, isRoad) {
        ...
        let uniforms = {
          uTravelLength: { value: options.length },
          uColor: { value: new Color(isRoad ? options.colors.roadColor : options.colors.islandColor) },
          uTime: this.uTime,
        };
        ...
        if (isRoad) {
          uniforms = Object.assign(uniforms, {
            ...
            uBrokenLinesColor: { value: new Color(options.colors.brokenLines) },
            uShoulderLinesColor: { value: new Color(options.colors.shoulderLines) },
...
export default HyperspeedComponent;
